...
| D16 | Repo integration | Shielding/report/canvas are **new Aran projects** consuming Aran.Model + ExtractedMazeGeometry; don't edit existing files from assumption (rule 9). | No guessing existing types. | DECIDED |
...
| D23 | Shielding geometry | New **`ShieldingGeometry`** record we own carrying the full physics input set — **S0 (throat)** and **S1 (corridor)** distinct, neutron **d1/d2** distinct from d_sec/d_L — derived from the confirmed geometry + `SectionGeometry`. New record (not an edit to `ExtractedMazeGeometry`). | O10; feeds doc 06. | DECIDED |
